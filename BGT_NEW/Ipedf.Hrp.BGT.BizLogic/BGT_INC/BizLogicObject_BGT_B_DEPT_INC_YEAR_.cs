...
namespace Ipedf.Web.BizLogic
{
	...
	public partial class BizLogicObject_BGT_B_DEPT_INC_YEAR
	{
        public BizLogicMsg Calculate(EntityObject_BGT_B_DEPT_INC_YEAR obj)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
                string retMsg = string.Empty;
                int succeed = HelperObject_BGT_B_DEPT_INC_YEAR.Calculate(obj.ID, out retMsg);
                msg.Message = retMsg;
        ...
        public override void PostUndoSubmit(EntityObject_BGT_B_DEPT_INC_YEAR entity, IDbTransaction transaction)
        {
            //删除对应明细项
            CauseObject_BGT_B_BUDGET_DATA_D pDelete = new CauseObject_BGT_B_BUDGET_DATA_D();
            pDelete.REF_TAB_NAME = "BGT_B_DEPT_INC_YEAR_D";
            CauseObject_BGT_B_DEPT_INC_YEAR_D pDetail = new CauseObject_BGT_B_DEPT_INC_YEAR_D();
            pDetail.BASE_ID = entity.ID;
            DisplayObject_BGT_B_DEPT_INC_YEAR_D[] detailResult = HelperObject_BGT_B_DEPT_INC_YEAR_D.Query(pDetail, transaction);
            foreach (DisplayObject_BGT_B_DEPT_INC_YEAR_D detail in detailResult)
            {
                pDelete.SetINValue("REF_TAB_ID", detail.ID);
            }
            HelperObject_BGT_B_BUDGET_DATA_D.Delete(pDelete, transaction);
        }
        public override void PostSubmit(EntityObject_BGT_B_DEPT_INC_YEAR entity, IDbTransaction transaction)
        {
            //检查主项是否存在
            CauseObject_BGT_B_BUDGET_DATA pbase = new CauseObject_BGT_B_BUDGET_DATA();
            pbase.BUDGET_CONTENTS_ID = "00120002";
            ...
            {
                //不存在则创建
                baseEntity.BUDGET_CONTENTS_ID = "00120002";
            ...
            foreach (DisplayObject_BGT_B_DEPT_INC_YEAR_D detail in detailResult)
            {
                ...
                detailEntity.REF_TAB_NAME = "BGT_B_DEPT_INC_YEAR_D";
            ...
            if (depts.Length == 0)
            {
                ...
                dataEntity.REF_TAB_NAME = "BGT_B_DEPT_INC_YEAR";
                HelperObject_BGT_B_BUDGET_DATA_DEPT.Save(dataEntity, transaction);
            }
        }
        public List<string> CheckImportService(EntityObject_BGT_B_DEPT_INC_YEAR obj, DataTable data)
        {
            if (LocalMode)
            {
        ...
        public BizLogicMsg ImportDataService(EntityObject_BGT_B_DEPT_INC_YEAR obj, DataTable data)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
        ...
        private List<string> CheckImportImpl(EntityObject_BGT_B_DEPT_INC_YEAR obj, DataTable data, IDbTransaction transaction)
        {
        ...
        private void ImportDataImpl(EntityObject_BGT_B_DEPT_INC_YEAR obj, DataTable data, IDbTransaction transaction)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                EntityObject_BGT_B_DEPT_INC_YEAR_D deptYearD = new EntityObject_BGT_B_DEPT_INC_YEAR_D();
                ...
                HelperObject_BGT_B_DEPT_INC_YEAR_D.Save(deptYearD, transaction);
            }
        }
