...
namespace Ipedf.Web.BizLogic
{
    ...
    public partial class BizLogicObject_BGT_B_DEPT_PAY_YEAR_FUND
    {
        public override void PostSave(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND entity, IDbTransaction transaction)
        {
            base.PostSave(entity, transaction);
        }
        public BizLogicMsg CommitSave(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND saveBaseEntity, EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND[] saveDetailEntities)
        {
            if (LocalMode)
            {
                ...
                using (IDbConnection connection = IDALProvider.IDAL.PopConnection())
                {
                    using (IDbTransaction transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            HelperObject_BGT_B_DEPT_PAY_YEAR_FUND.Save(saveBaseEntity, transaction);
                            decimal money = 0;
                          //  decimal control_money = 0;
                           ...
                           // saveBaseEntity.CONTROL_MONEY = control_money;
                            ...
                            HelperObject_BGT_B_DEPT_PAY_YEAR_FUND.Update(saveBaseEntity, transaction);
        ...
        public BizLogicMsg CommitUpdate(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND updateBaseEntity, EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND[] saveDetailEntities, EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND[] updateDetailEntities, EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND[] deleteDetailEntities)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
                using (IDbConnection connection = IDALProvider.IDAL.PopConnection())
                {
                    using (IDbTransaction transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            HelperObject_BGT_B_DEPT_PAY_YEAR_FUND.Update(updateBaseEntity, transaction);
                            decimal money = 0;
                           // decimal control_money = 0;
                            //平台自动生成代码
                            foreach (EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND obj in saveDetailEntities)
        ...
        #region 导入导出功能
        public List<string> CheckService1(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data)
        {
            if (LocalMode)
            {
        ...
        public BizLogicMsg ImportService1(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
        ...
        private List<string> CheckImpl1(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data, IDbTransaction transaction)
        {
        ...
        private void ImportImpl1(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data, IDbTransaction transaction)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
        ...
        public List<string> CheckService2(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data)
        {
            if (LocalMode)
            {
                List<string> colMsg = null;
        ...
        public BizLogicMsg ImportService2(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
        ...
        private List<string> CheckImpl2(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data, IDbTransaction transaction)
        {
            List<string> colMsg = new List<string>();              //批量保存每行数据保存结果, 用于回写到数据源文件, 且列表第一条数据作为数据源是否有错误的标识
            colMsg.Add("Succeed");
            int errNum = 0;      //记录每行数据的错误数量
        ...
        private void ImportImpl2(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data, IDbTransaction transaction)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND fundD = new EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND();
        ...
        public List<string> CheckService3(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data)
        {
            if (LocalMode)
            {
                List<string> colMsg = null;
        ...
        public BizLogicMsg ImportService3(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
        ...
        private List<string> CheckImpl3(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data, IDbTransaction transaction)
        {
            List<string> colMsg = new List<string>();              //批量保存每行数据保存结果, 用于回写到数据源文件, 且列表第一条数据作为数据源是否有错误的标识
            colMsg.Add("Succeed");
            int errNum = 0;      //记录每行数据的错误数量
        ...
        private void ImportImpl3(EntityObject_BGT_B_DEPT_PAY_YEAR_FUND obj, DataTable data, IDbTransaction transaction)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND fundD = new EntityObject_BGT_B_DEPT_PAY_YEAY_D_FUND();
