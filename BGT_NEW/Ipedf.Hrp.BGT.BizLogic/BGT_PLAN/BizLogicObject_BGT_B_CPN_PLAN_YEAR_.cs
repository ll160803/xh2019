...
namespace Ipedf.Web.BizLogic
{
    ...
    public partial class BizLogicObject_BGT_B_CPN_PLAN_YEAR
    {
        public BizLogicMsg Calculate(EntityObject_BGT_B_CPN_PLAN_YEAR obj)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
                string retMsg = string.Empty;
                int succeed = HelperObject_BGT_B_CPN_PLAN_YEAR.Calculate(obj.ID, out retMsg);
                msg.Message = retMsg;
        ...
        public List<string> CheckImportService(EntityObject_BGT_B_CPN_PLAN_YEAR obj, DataTable data)
        {
            if (LocalMode)
            {
        ...
        public BizLogicMsg ImportDataService(EntityObject_BGT_B_CPN_PLAN_YEAR obj, DataTable data)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
        ...
        private List<string> CheckImportImpl(EntityObject_BGT_B_CPN_PLAN_YEAR obj, DataTable data, IDbTransaction transaction)
        {
        ...
        private void ImportDataImpl(EntityObject_BGT_B_CPN_PLAN_YEAR obj, DataTable data, IDbTransaction transaction)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                EntityObject_BGT_B_CPN_PLAN_YEAR_D cpnYearD = new EntityObject_BGT_B_CPN_PLAN_YEAR_D();
                ...
                cpnYearD.ITEM_TYPE_ID = items[0].ITEM_TYPE_ID;
                cpnYearD.ITEM_VALUE = ResolveDataCellNumber(data.Rows[i][1]);
                cpnYearD.BUDGET_BASE = ResolveDataCellNumber(data.Rows[i][2]);
                HelperObject_BGT_B_CPN_PLAN_YEAR_D.Save(cpnYearD, transaction);
            }
        }
