...
namespace Ipedf.Web.BizLogic
{
	...
	public partial class BizLogicObject_BGT_B_DEPT_INC_MON_FACT
	{
        public BizLogicMsg Archive(EntityObject_BGT_B_DEPT_INC_MON_FACT obj)
        {
            if (LocalMode)
            {
                ...
                try
                {
                    HelperObject_BGT_B_DEPT_INC_MON_FACT.ApplyFactData(obj.ID, out outmsg);
                }
        ...
        public BizLogicMsg PrepareItemList(EntityObject_BGT_B_DEPT_INC_MON_FACT obj)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
                ...
                using (IDbConnection connection = IDALProvider.IDAL.PopConnection())
                {
                    using (IDbTransaction transaction = connection.BeginTransaction())
                    {
                        ...
                        {
                            //先删除
                            CauseObject_BGT_B_DEPT_INC_MON_FACT_D pdelete = new CauseObject_BGT_B_DEPT_INC_MON_FACT_D();
                            pdelete.BASE_ID = obj.ID;
                            HelperObject_BGT_B_DEPT_INC_MON_FACT_D.Delete(pdelete, transaction);
                            //再新增
                            CauseObject_BGT_B_DEPT_MON_EXE_D pSource = new CauseObject_BGT_B_DEPT_MON_EXE_D();
                            pSource.SetCustomCondition(string.Format(" and BGT_B_DEPT_MON_EXE_D.BASE_ID in(select t.id from BGT_B_DEPT_MON_EXE t where t.BUDGET_DEPT_ID='{0}' and t.BUDGET_CONTENTS_ID = '{1}' and t.BUDGET_YEAR = (select id from BGT_D_BUDGET_YEAR where BUDGET_YEAR = substr('{2}',0,4) ))", obj.BUDGET_DEPT_ID, "00120002", obj.FINANCE_CYCLE));//收入预算
                            ...
                            {
                                EntityObject_BGT_B_DEPT_INC_MON_FACT_D factd = new EntityObject_BGT_B_DEPT_INC_MON_FACT_D();
                                factd.BASE_ID = obj.ID;//关联主表
                                factd.ITEM_ID = item.ITEM_ID;
                                HelperObject_BGT_B_DEPT_INC_MON_FACT_D.Save(factd, transaction);
                            }
                            transaction.Commit();
                        }
                        catch (Exception expt)
        ...
        public BizLogicMsg ImportDataService(EntityObject_BGT_B_DEPT_INC_MON_FACT obj, string budgetYear, DataTable data)
        {
            if (LocalMode)
            {
                BizLogicMsg msg = new BizLogicMsg();
        ...
        private void ImportDataImpl(EntityObject_BGT_B_DEPT_INC_MON_FACT obj, string budgetYear, DataTable data, IDbTransaction transaction)
        {
            Dictionary<string, string> dicSource = new Dictionary<string, string>();    //用于保存数据来源类型
