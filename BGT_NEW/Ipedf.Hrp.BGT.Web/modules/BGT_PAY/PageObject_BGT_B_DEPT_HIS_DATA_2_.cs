...
namespace Ipedf.Web
{
    public partial class PageObject_BGT_B_DEPT_HIS_DATA_2
    {
        protected override void PrepareDetailInput()
        {
            this.gvDetailList.DataKeyNames = PrepareDetailGridKeyNames();
            this.gvDetailList.DataSource = CreateEmptyDataSource(typeof(DisplayObject_BGT_B_DEPT_HIS_DATA_D_EXT), NewInputRowCount);
        ...
        protected override object PrepareDetailDataSource()
        {
            ...
            //平台自动生成代码
            CauseObject_BGT_B_DEPT_HIS_DATA_D cause = new CauseObject_BGT_B_DEPT_HIS_DATA_D();
            if (ViewState["CUR_OBJ"] != null)
                cause.BASE_ID = (ViewState["CUR_OBJ"] as EntityObject_BGT_B_DEPT_HIS_DATA).ID;
            OrderByParameter orderBy = new OrderByParameter();
            orderBy.Asc = true;
            orderBy.OrderBy = "ITEM_ID_NAME";
            dataSource = BizLogicObject_BGT_B_DEPT_HIS_DATA_D.Proxy.QueryFinanceExt(cause, null, orderBy);
            System.Collections.Generic.List<DisplayObject_BGT_B_DEPT_HIS_DATA_D_EXT> list = new System.Collections.Generic.List<DisplayObject_BGT_B_DEPT_HIS_DATA_D_EXT>();
            list.AddRange((DisplayObject_BGT_B_DEPT_HIS_DATA_D_EXT[])dataSource);
            list.AddRange(CreateEmptyDataSource<DisplayObject_BGT_B_DEPT_HIS_DATA_D_EXT>(5));
        ...
        public override void ShowEditState(object entity)
        {
            ...
            //2016-07-28
            if ((entity as EntityObject_BGT_B_DEPT_HIS_DATA).STATE == (decimal)Ipedf.Common.数据状态.编辑中)
            {
                btnScan.Visible = btnUpload.Visible = this.command[Ipedf.Web.Control.ButtonType.Save].Enabled = true;
            }
            else
        ...
        public void ImportClick(string filePath)
        {
            try
            {
                ...
                List<string> colMsg = BizLogicObject_BGT_B_DEPT_HIS_DATA.Proxy.CheckPayService(baseEntity, data);
                StringBuilder strResult = new StringBuilder();
                //ImportUtil.AppendCheckResultToExcel(filePath, colMsg);
                if (colMsg[0] == "Succeed")
                {
                    BizLogicMsg msg = BizLogicObject_BGT_B_DEPT_HIS_DATA.Proxy.ImportPayService(baseEntity, data);
                    if (msg.Succeed)
                    {
