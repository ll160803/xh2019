...
namespace Ipedf.Web.DataAccess
{
    ...
    {
        public static DisplayObject_V_BGT_PAY_BUDGET_DATA_EXT[] QueryExt(CauseObject_V_BGT_PAY_BUDGET_DATA cause, PagingParamter paging, OrderByParameter order)
        {
            ...
            string sSql = @" select  V_BGT_PAY_BUDGET_DATA.BUDGET_DATA_ID, 
                                     V_BGT_PAY_BUDGET_DATA.ITEM_ID, 
                                     V_BGT_PAY_BUDGET_DATA.DEPT_VALUE, 
                                     V_BGT_PAY_BUDGET_DATA.DIFF_VALUE, 
                                     V_BGT_PAY_BUDGET_DATA.BUDGET_YEAR, 
                                     V_BGT_PAY_BUDGET_DATA.CPN_VALUE, 
                                     BGT_D_BUDGET_ITEM_ITEM_ID.NAME as ITEM_ID_NAME, 
                                     BGT_D_BUDGET_YEAR_BUDGET_YEAR.BUDGET_YEAR as BUDGET_YEAR_NAME,  
                                     '0001000x' as UNIT_ID, 
                                     '元' as UNIT_ID_NAME   
                                     from V_BGT_PAY_BUDGET_DATA 
                                     left join BGT_D_BUDGET_ITEM   BGT_D_BUDGET_ITEM_ITEM_ID 
                                     on V_BGT_PAY_BUDGET_DATA.ITEM_ID = BGT_D_BUDGET_ITEM_ITEM_ID.ID 
                                     left join BGT_D_BUDGET_YEAR   BGT_D_BUDGET_YEAR_BUDGET_YEAR 
                                     on V_BGT_PAY_BUDGET_DATA.BUDGET_YEAR = BGT_D_BUDGET_YEAR_BUDGET_YEAR.ID 
                                     where (1=1)";
            ...
            DisplayObject_V_BGT_PAY_BUDGET_DATA_EXT dataObj = null;
            ArrayList objArray = new ArrayList();
            while (sqlReader.Read())
            {
                dataObj = new DisplayObject_V_BGT_PAY_BUDGET_DATA_EXT();
            ...
            #endregion
            DisplayObject_V_BGT_PAY_BUDGET_DATA_EXT[] objs = new DisplayObject_V_BGT_PAY_BUDGET_DATA_EXT[objArray.Count];
