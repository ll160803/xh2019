...
<head id="Head1" runat="server">
    ...
    $("input[requiredh='true']").each(function() {
        if ($(this).val().length == 0) {
            $.messager.alert('上传附件文件', '必须上传物料检验证书!' ,'warning');
            //alert($(this).attr("id"));
            flag = 1;
            return false;
        }
    ...
    if (flag == 1) {
        return false;
	        ...
	        $('#ff').form('submit', {
	            url: "<%: Url.Content("~/System/SCM_D_MATER/Edit/")%>",
                 type: "Post",
                dataType: "html",
                onsubmit: function () {
                ...
                success: function (data) {
                   // var data=JSON.parse(data1);//eval("("+data1+")");
            ...
            $("#MATER_ID").combogrid({
                ...
                keyHandler: {
                 ...
                 onLoadSuccess:function(){
                       // $("#MATER_ID").combogrid("setText",);
                         if (!$("#MATER_ID").combogrid('grid').datagrid('getSelected'))  //不再当前页
                          $("#MATER_ID").next().find('input.combo-text').val('<%:ViewBag.MaterName %>');
                    }
            });
                    ...
                    $('#uploadexcel').form('submit', {
                        ...
                        success: function (data) {
                            ...
                            if (data != "0") {
                                ...
                                $.messager.alert('文件上传', '文件大小最大支持3M', 'info');
...
<body>
    <div class="easyui-panel" title="资质上传" style="width: 1000px">
        <div style="padding: 10px 60px 20px 60px">
            ...
            <fieldset>
                <legend>
                   <%if(Model.STATE!=2){ %> <input class="a2 f2" type="button" onclick="submitForm();" value="保存" /><%} %>
                ...
                <table cellpadding="5">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" class="easyui-validatebox  textbox sTd" value="<%:Model.CHARGE %>" id="CHARGE" name="CHARGE" data-options="required:true"></input>
                        </td>
                    </tr>
                    <tr>
                        <td class="fTd">
                        ...
                        <td>
                         <a href="#" class="easyui-linkbutton" name="btnSave" <%if(!string.IsNullOrEmpty(ViewBag.FName)){ %> style="display: none;"<%} %>;" id="btnSave1"
                            onclick="UpLoad(1,'BANK_NAME_Att','hide_BANK_NAME','btnSave1')">上传附件</a><span id="BANK_NAME_Att"><%:ViewBag.FName %><% if (Model.STATE!=2&&!string.IsNullOrEmpty(ViewBag.FName)){%><a
