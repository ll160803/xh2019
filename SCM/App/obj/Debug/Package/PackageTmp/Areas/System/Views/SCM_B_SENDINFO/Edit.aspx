<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Edit.Master" Inherits="System.Web.Mvc.ViewPage<Ipedf.Web.Entity.DisplayObject_SCM_B_SENDINFO>" %>
    ...
    var WERKS =  <%:ViewData["WERKS"] %>
        $(function () {
            ...
            $("#SEND_DEPART").combogrid({
                ...
                editable: true,
                rownumbers: true,
                url: '<%: Url.Content("~/System/SCM_D_SENDDEPART/GetComBoxData")%>'+'?type='+WERKS,
            ...
            if (!$("#MATNR").combogrid('grid').datagrid('getSelected'))  //不再当前页
                $("#MATNR").next().find('input.combo-text').val('<%=Model.TXZ01 %>');
            if (!$("#SEND_DEPART").combogrid('grid').datagrid('getSelected'))  //不再当前页
                $("#SEND_DEPART").next().find('input.combo-text').val('<%=Model.SEND_DEPART_NAME %>');
        });
