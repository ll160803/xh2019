using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Ipedf.Web.Entity
{
    public partial class DisplayObject_MDM_D_DEPT
    {
        public string FID { get; set; }
        public string SID { get; set; }
        public string TID { get; set; }
        public string FouthID { get; set; }

        public string ID_5 { get; set; }
        public string ID_6 { get; set; }
        public string ID_7 { get; set; }
        public string ID_8 { get; set; }
        public string ID_9 { get; set; }
        public string ID_11 { get; set; }
        public string ID_12 { get; set; }
        public string ID_13 { get; set; }
        public string ID_14 { get; set; }
        public string ID_15 { get; set; }
        public string ID_16 { get; set; }
        public string ID_17 { get; set; }
        public string ID_18 { get; set; }
        public string ID_19 { get; set; }

        public string CONTRACT_DATE { get; set; }

        public int YH { get; set; }
    }
}
