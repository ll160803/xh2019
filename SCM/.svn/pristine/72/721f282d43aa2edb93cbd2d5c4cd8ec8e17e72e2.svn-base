...
namespace Ipedf.Web.Common
{
    public class NPOIWriteExcel {
        ...
        public static string WriteExcelU<T>(Action<List<T>,string, Dictionary<int, string>, Dictionary<int, int[]>, int, ISheet, IWorkbook> action, List<T> queryData,string arrStr, Dictionary<int, string> dicFields, Dictionary<int, int[]>  dicRange, int startIndex = 1, string xlsPath = @"~/up/供应计划.xls", string SavePath1 = @"up/",int nPathNum=0, string xlsName = "供应计划", string sheetName = "Sheet1")
        {
            HSSFWorkbook _book = new HSSFWorkbook();
            FileStream file = new FileStream(System.Web.HttpContext.Current.Server.MapPath(xlsPath), FileMode.Open, FileAccess.Read);
            IWorkbook hssfworkbook = new HSSFWorkbook(file);
            ...
            action(queryData,arrStr, dicFields, dicRange, startIndex, sheet, hssfworkbook);
            ...
            string strPath = "";
            for (int p = 0; p < nPathNum; p++)
            {
                strPath += "../";
            }
            return string.Format(strPath + "{1}{0}.xls", guid, SavePath1);
        }
        ...
        public static void GenerateRowAndWriteDataU<T>(List<T> queryData,string arrStr, Dictionary<int, string> dicFields, Dictionary<int, int[]> dicRange, int startIndex, ISheet sheet, IWorkbook hssfworkbook)
        {
            //标题行 默认  开始行  
            int stratIndex = startIndex;//开始行 
            int RowIndex = stratIndex + 1;//excel输出到的当前行数
            ...
            HSSFRow dataRowColumn = GetCurrentRow(sheet, startIndex-1);
            NPOI.SS.UserModel.ICell cell = dataRowColumn.CreateCell(0);
            cell.SetCellValue(arrStr);  
            ...
            if (queryData.Count > 0 && dicRange!=null)
                ForDataRange(queryData, dicFields,dicRange, startIndex,sheet);
        }

        public static void ForDataRange<T>(List<T> queryData, Dictionary<int, string> dicFields, Dictionary<int, int[]> dicRange, int stratIndex, ISheet sheet)
        {
            int nRangRow = 0;
            int nRangRow1 = 0;
            ...
            for (int i = 0; i < dicRange.Keys.Count; i++)
            {
                itemCol = dicRange[i];
                nRangRow = stratIndex + 1;
                nRangRow1 = stratIndex + 1;
                nRangI = stratIndex + 1;
                nRangJ = stratIndex + 1;
                var fieldName = dicFields[dicRange[i][0]];
                Type columnType = typeof(string);
                for (int k = 0; k < queryData.Count; k++)
                {
                    
                    var cellValue = GetValueByFieldOrPerperty<T>(queryData[k], fieldName, ref columnType);
                    if (cellValue == null)
                    {
                        continue;
                    }
                    else
                    {
                        ...
                        {
                            strValueNew = (string)cellValue;
                            if (i != 0)
                                strValueNew += dicKeys[k];

                            if (strValue == strValueNew)
                            {
                                nRangRow1 += 1;
                            }
                            else
                            {
                                IsTrue = true;
                                foreach(int nCol in itemCol)
                                {
                                    SetCellRangeAddress(sheet, nRangRow, nRangRow1, nCol, nCol);
                                }
                                nRangI = nRangRow1 + 1;
                                nRangRow = nRangRow1 + 1;
                                nRangRow1 = nRangRow1 + 1;
                                strValue = (string)cellValue;
                                if (i != 0)
                                    strValue += dicKeys[k];
                            }
                ...
                if (itemCol != null)
                {
                    foreach (int nCol in itemCol)
                    {
                        SetCellRangeAddress(sheet, IsTrue ? nRangI : nRangI - 1, nRangJ, nCol, nCol);
                    }
                }
            }
