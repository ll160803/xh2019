...
function GetWorkFlowStateGuidByUrl(url) {
    $('#WorkFlowStautsGuid').combobox({
        url: '../../WC_SYSTEM_WORKFLOWSTAUTS/GetWorkFlowStateGuidByUrl?url='+url,
        width: '137',
        panelWidth: '300',
        valueField: 'WorkFlowStautsGuid',
        textField: 'WorkFlowStautsName',
        editable: false
    });
}
function GetSysDepartmentByPerId(id) {
...
function showModalOnly_New(me, url) { //弹出窗体 ,单选
    var hidden = document.getElementById(me); //获取隐藏的控件
