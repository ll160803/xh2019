...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function ShowCenterHtml(obj) {
            $("#centerFrame").attr("src", obj.attr("rel"));
        }
        function CallBackByID(sysId) {
            $.ajax({
                url: '<%: Url.Content("~/Home/CreateItem/")%>' + sysId,
                type: "Post",
                dataType: "html",
                success: function (formHtml) {
                    $("#leftMenu").empty();
                    $("#leftMenu").html(formHtml);
                    $.parser.parse("#west");
                }
            });
...
<body class="easyui-layout">
    ...
    <div region="north" split="false" border="false" style="overflow: hidden; height: 60px;background: url(Images/Login/title_bg.jpg) repeat-x;
        font-family: Verdana, 微软雅黑,黑体">
        <div style=" width:100%;height: 60px;background: url(Images/Login/title.jpg) no-repeat;" >
        <span style="float: right; padding-right: 20px; padding-top: 35px;">欢迎[<%= ViewData["PersonName"]%>]&nbsp;[<%= ViewData["Department"]%>]&nbsp;<a
            href="#" class="index_alink" id="ChangePassword">修改密码</a> <a class="index_alink"
                href="#" id="loginOut">安全退出</a> </span></div>
    </div>
    ...
    <div id="mainPanle" region="center" class="easyui-panel" title=' ' data-options="iconCls:'icon-save'"
        style="background: #eee; overflow-y: hidden;">
        <iframe name="centerFrame" id="centerFrame" height="100%" width="100%" border="0"
            frameborder="0" src='home/welcome'>浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。</iframe>
    </div>
